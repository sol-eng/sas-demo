---
title: "Mixed Effects Modeling with R and SAS"
...
This document serves as an example on how to efficiently mix and match R and SAS code. The general idea here is do demonstrate that a data scientist can pick & choose the tool of choice for each task and transparently exchange data between the languages.

Specifically we will

-   create a synthetic dataset in R
-   run the analysis in SAS
-   visualise the results in R

We are making use of a new R package named `sasquatch` (cf. [ropensci](https://docs.ropensci.org/sasquatch/)) that also contains a quarto extension.
...
```{r}
#| label: copy-data-from-SAS-to-R

# Get the fixed effects results as R dataframe
fixed_effects <- sas_to_r("fixed_effects")


# Get the mixed results as R dataframe
mixed_results <- sas_to_r("mixed_results")


# Get the variance components results as R dataframe
variance_components <- sas_to_r("variance_components")
```
