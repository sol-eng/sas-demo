    ...
    do study_id=1 to 5;
        do site_id=1 to 2;
            do subject_id=1 to 20; /* Reduced from 50 to 20 */
                ...
                unique_subject_id=(study_id * 100) + (site_id * 20) +
                    subject_id;
...
proc sql;
    select count(*) as total_observations, count(distinct study_id) as
        num_studies, count(distinct site_id) as num_sites, count(distinct
        unique_subject_id) as num_subjects from large_study_data;
quit;

/* Simple PROC MIXED analysis with residuals output */
proc hpmixed data=large_study_data;
